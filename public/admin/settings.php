...
<form method="post" enctype="multipart/form-data">
    ...
    <div class="card bg-white shadow-sm mb-4">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-3 mb-3"><label class="form-label"><?= h(admin_t('language', $config)) ?></label><select name="language" class="form-select">
                        <option value="auto" <?= ($config['language'] ?? 'auto') === 'auto' ? 'selected' : '' ?>><?= h(admin_t('language_auto', $config)) ?></option>
                        <option value="de" <?= ($config['language'] ?? 'auto') === 'de' ? 'selected' : '' ?>><?= h(admin_t('german', $config)) ?></option>
                        <option value="en" <?= ($config['language'] ?? 'auto') === 'en' ? 'selected' : '' ?>><?= h(admin_t('english', $config)) ?></option>
                    </select></div>
                <div class="col-md-3 mb-3"><label class="form-label"><?= h(admin_t('theme', $config)) ?></label><select name="theme" class="form-select">
                        <option value="classic" <?= ($config['theme'] ?? 'classic') === 'classic' ? 'selected' : '' ?>>Classic</option>
                        <option value="minimal" <?= ($config['theme'] ?? 'classic') === 'minimal' ? 'selected' : '' ?>>Minimal</option>
                        <option value="glass" <?= ($config['theme'] ?? 'classic') === 'glass' ? 'selected' : '' ?>>Glass</option>
                    </select></div>
                <div class="col-md-3 mb-3"><label class="form-label"><?= h(admin_t('color_mode', $config)) ?></label><select name="theme_mode" class="form-select">
                        <option value="auto" <?= theme_mode($config) === 'auto' ? 'selected' : '' ?>><?= h(admin_t('auto', $config)) ?></option>
                        <option value="light" <?= theme_mode($config) === 'light' ? 'selected' : '' ?>><?= h(admin_t('light', $config)) ?></option>
                        <option value="dark" <?= theme_mode($config) === 'dark' ? 'selected' : '' ?>><?= h(admin_t('dark', $config)) ?></option>
                    </select></div>
                <div class="col-md-3 mb-3 d-flex align-items-end">
                    <div class="form-check form-switch"><input class="form-check-input" type="checkbox" name="pwa_enabled" id="pwa_enabled" <?= !empty($config['pwa_enabled']) ? 'checked' : '' ?>><label class="form-check-label" for="pwa_enabled"><?= h(admin_t('pwa_enable', $config)) ?></label></div>
                </div>
            </div>
        </div>
    </div>
