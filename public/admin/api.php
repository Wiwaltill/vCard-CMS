...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header('Location: /admin/api?saved=1');
    exit;
}
...
<div class="card shadow-sm">
    <div class="card-body">
        ...
        <hr>
        <p class="mb-2"><?= h(admin_t('endpoints_with_header', $config)) ?> <code>X-API-Token</code>:</p>
        <pre class="bg-body-tertiary border p-3 rounded text-body font-monospace">
            GET    /api/contacts
            GET    /api/contacts/{id}
            POST   /api/contacts
            PUT    /api/contacts/{id}
